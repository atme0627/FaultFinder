...
**リファクタリング時の変更単位**：
- 同一の変更（変数名の置換、import の追加等）は一度にまとめて行う
- 複雑なリファクタリングでも、同じ種類の機械的な変更は一括で実施
- ただし、異なる種類の変更は分けて段階的に実施し、各段階でテストを実行
...
## 段階的リファクタリング（Phase 形式）の進め方

複数の Phase に分かれたリファクタリング計画を実行する場合：

### 各 Phase での議論プロセス（必須）

**実装を始める前に、必ず Phase の詳細を説明し議論を行う。**
...
2. **議論フェーズ**
   - ユーザーからの質問に回答
   - 代替案の検討（既存の仕組みで代用できないか等）
   - 不要な変更の削除やスキップの判断

3. **承認フェーズ**
   - ユーザーの明示的な承認を得てから実装開始
   - 承認なしに実装を始めない
...
5. **記録フェーズ**
   - 議論の結果をリファクタリング計画に反映
   - スキップした Phase やその理由を記録

### 計画の柔軟な修正

- 議論の結果、Phase のスキップや統合は積極的に行う
- 「計画通りに進める」より「最適な結果を得る」を優先
- 変更があった場合は計画書を更新
...
**タスクが完了するたびに、必ず議事録を `docs/design-notes/` に作成すること。**
...
例: `2026-02-03-susp-isfield-removal.md`
...
### Single Task Commit の原則
- **1コミット = 1タスク**を徹底する（single task commit）
- 複数の目的を含む composite commit はできるだけ避ける
- 例: 「リファクタ + 新機能追加」→ 別々のコミットに分ける
- 例: 「パッケージ移動 + 命名変更」→ 同一目的なら1コミットで可
...
### `docs/plans/`
**これから取り組む課題の計画書**を保管する場所。

- 実装前の設計・計画段階で作成
- 複数ステップにわたる大きな変更の計画
- 完了したら報告書を `design-notes/` に作成し、計画ファイルは削除
...
- 将来対応が必要な課題
- 調査結果や原因分析
- 解決したら報告書を `design-notes/` に作成し、issue ファイルは削除

**優先度別サブディレクトリ**:
- `high/`: 高優先度（バグ、機能不全、セキュリティ問題）
- `medium/`: 中優先度（保守性、テスタビリティ、拡張性の向上）
- `low/`: 低優先度（スタイル統一、設計改善、ドキュメント改善）

### `docs/design-notes/`
**完了したタスクの議事録・報告書**を保管する場所。
