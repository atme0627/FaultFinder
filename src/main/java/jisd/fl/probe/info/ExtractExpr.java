...
public class ExtractExpr {
    static class ExtractExprAssignment {
        static Expression extractExprAssign(boolean deleteParentNode, Statement stmt){
            try {
                Expression result = ExtractExpr.ExtractExprAssignment.extractExpressionFromStatement(stmt);
                return ExtractExpr.ExtractExprAssignment.finalizeResult(result, deleteParentNode);
        ...
        static Expression extractExpressionFromStatement(Statement stmt) {
            ...
            if (assignExpr.isPresent()) {
                return ExtractExpr.ExtractExprAssignment.extractFromAssignExpr(assignExpr.get());
            }
            ...
            if (vdExpr.isPresent()) {
                // 代入文がひとつであると仮定
                return ExtractExpr.ExtractExprAssignment.extractFromVariableDeclaration(vdExpr.get());
            }
